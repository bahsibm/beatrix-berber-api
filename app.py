...
def cancel_appointment():
    """Randevu iptal et (var olan kaydı iptal et veya yeni iptal kaydı oluştur)."""
    data = request.get_json()
    ...
    try:
        ...
        if existing:
            ...
            conn.execute(
                'INSERT INTO appointments (date, time, name, phone, status, note) VALUES (?, ?, ?, ?, ?, ?)',
                (date_str, time_str, 'İptal', '-', 'cancelled', note)
            )
...
@app.route('/api/uncancel-slot', methods=['POST'])
def uncancel_slot():
    """İptal edilen saat dilimini tekrar müsait yap."""
    data = request.get_json()
    if not data:
        return jsonify({"message": "Geçersiz istek verisi."}), 400
    ...
    if not all([date_str, time_str]):
        return jsonify({"message": "Tarih ve saat gereklidir."}), 400

    conn = get_db()
    try:
        conn.execute('DELETE FROM appointments WHERE date = ? AND time = ?', (date_str, time_str))
        conn.commit()
        conn.close()
        return jsonify({"message": f"{time_str} saati tekrar müsait yapıldı."}), 200
    except Exception as e:
        conn.close()
        return jsonify({"message": f"Hata: {str(e)}"}), 500
